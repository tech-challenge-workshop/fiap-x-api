...
| `test/jest-e2e.json` declares no `testPathIgnorePatterns`, unlike the other three services, so AppleDouble sidecars would be collected as test files | `test/jest-e2e.json:1` | Harmless in CI, since a clean checkout has no `._` files, but the inconsistency hides a real difference between the four repositories | Documented here rather than changed: altering the e2e runner config is outside this slice's stated scope. Raised as a follow-up. |
