# FIAP X API CI Pipeline Specification

## Problem Statement

The API repository has no automated verification. Its gates (`npm run lint`, `npm test`, `npm run test:e2e`, `npm run build`) exist but run only when a developer remembers to run them, so a regression can reach `main` unnoticed.
...
| End-to-end suite in CI | Runs in the same job as the unit tests | The API e2e suite uses an in-memory Catalog double and reaches no external service. | y |
